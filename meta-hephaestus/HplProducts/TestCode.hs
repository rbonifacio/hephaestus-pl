...
transform (ComponentTransformation x0) x1 x2
  = transformComponent  x0 x1 x2
 
mkEmptyInstance ::
                  FeatureConfiguration -> SPLModel -> InstanceModel
mkEmptyInstance fc spl = InstanceModel{featureConfiguration = fc, components = [], buildEntries= [], preProcessFiles=[]}
...
main
  = do cDir <- getCurrentDirectory
       ...
       let spl = SPLModel{featureModel = fm, splMappings = comppl}
